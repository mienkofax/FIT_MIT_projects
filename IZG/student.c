...
void studrenDrawTriangle(S_Renderer *pRenderer,
	...
	int x3, int y3,
	S_Coords *tCoords,
	double co1, double co2, double co3
)
{
	...
	for (y = miny; y <= maxy; ++y)
	{
		...
		for (x = minx; x <= maxx; ++x)
		{
			if (e1 >= 0 && e2 >= 0 && e3 >= 0)
			{
				/* interpolace pomoci barycentrickych suradnic */
				w1 = alpha * e2;
				w2 = beta * e3;
				w3 = gamma * e1;

				/* interpolacia z-souradnice */
				z = w1 * v1->z + w2 * v2->z + w3 * v3->z;

				textureX = (w1 * tCoords[0].x / co1 + w2 * tCoords[1].x / co2 + w3 * tCoords[2].x / co3) / (w1 / co1 + w2 / co2 + w3 / co3);
				textureY = (w1 * tCoords[0].y / co1 + w2 * tCoords[1].y / co2 + w3 * tCoords[2].y / co3) / (w1 / co1 + w2 / co2 + w3 / co3);
...
void studrenProjectTriangle(S_Renderer *pRenderer, S_Model *pModel, int i, float n)
{
	...
	co1 = trProjectVertex(&u1, &v1, &a);
	co2 = trProjectVertex(&u2, &v2, &b);
	co3 = trProjectVertex(&u3, &v3, &c);
...
S_RGBA studrenTextureValue(S_StudentRenderer * pRenderer, double u, double v)
{
	...
	uu = v * (pRenderer->width - 1);
	vv = u * (pRenderer->height - 1);

	/* najdenie najblizsich suradnic */
	...
	/* vahy pre jednotlive koeficienty */
	tmp1 = (1.0 - tmpX) * 256;
	tmp2 = 1.0 - tmpY;
	w1 = tmp1 * tmp2;
	w2 = tmpX * tmp2 * 256;
