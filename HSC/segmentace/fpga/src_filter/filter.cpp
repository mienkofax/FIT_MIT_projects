...
typedef ac_int<PIXEL_WIDTH,false> t_pixel_sw;
...
***************************************************************************/
void buffer(t_pixel_sw din, ac_int<9, false> c, t_pixel_sw *col_window)
{
...
***************************************************************************/
void clip_window(ac_int<8, false> r, ac_int<9, false> c, t_pixel_sw *iw, t_pixel_sw *ow)
{
...
void pixel_processing(t_pixel data_in, t_pixel &data_out, t_mcu_data mcu_data[MCU_SIZE])
{
	...
	static   ac_int<1,false>   last;
	t_pixel                    filtered, window[9];
	...
	if (mod10 == 0)
		mcu_data[FPGA_HISTOGRAM + filtered]++;

	data_out = thresholding(filtered, threshold);
